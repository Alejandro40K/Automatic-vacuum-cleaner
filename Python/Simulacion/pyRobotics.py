...
import numpy as np
import glob
from pyvistaqt import BackgroundPlotter  # 👈 Nueva importación
...
class robotics:
    ...
    def configureScene(self, bounds, window_size=(1024, 768), title="Python Robotics"):
        self.bounds = bounds
        self.plotter = BackgroundPlotter(window_size=window_size, title=title)  # 👈 Corregido
    ...
    def startSimulation(self, step=1, ts=0.1):
        cpos = [(-8, -8, 8),  # zoom x y z
                ...
                (0.28, 0.28, 0.28)]
        #self.plotter.show_bounds(grid='True', location='outer', color='#000000',
                                # bounds=self.bounds, xlabel='x [m]', ylabel='y [m]', zlabel='z [m]')
